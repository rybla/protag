module Protag.Variant
  ...
  , variantBoundedEnums
  , class EncodeJson_Variant
  , encodeJson_Variant
  , class DecodeJson_Variant
  , decodeJson_Variant
  , module Exports
  ) where
...
instance
  ( RowToList r rl
  , EncodeJson_Variant r rl
  ) =>
  EncodeJson (Variant r) where
  encodeJson = encodeJson_Variant @r @rl

class EncodeJson_Variant :: Row Type -> RowList Type -> Constraint
class EncodeJson_Variant r rl | rl -> r where
  encodeJson_Variant :: Variant r -> Json

instance EncodeJson_Variant () RL.Nil where
  encodeJson_Variant = case_
...
instance
  ...
  EncodeJson_Variant r' (RL.Cons k v rl) where
  encodeJson_Variant = on (Proxy @k) encodeJson (encodeJson_Variant @r @rl)

instance
  ( RowToList r rl
  , DecodeJson_Variant r r rl
  ) =>
  DecodeJson (Variant r) where
  decodeJson = decodeJson_Variant @r @r @rl

class DecodeJson_Variant :: Row Type -> Row Type -> RowList Type -> Constraint
class DecodeJson_Variant r0 r rl | rl -> r where
  decodeJson_Variant :: Json -> JsonDecodeError \/ Variant r

instance DecodeJson_Variant r0 () RL.Nil where
  decodeJson_Variant _json = throwError $ TypeMismatch $ "expected a " <> show (Proxy @(Variant r0))

instance
  ( IsSymbol k
  , Cons k v r r'
  , DecodeJson v
  , DecodeJson_Variant r0 r rl
  ) =>
  DecodeJson_Variant r0 r' (RL.Cons k v rl) where
  decodeJson_Variant json = case decodeJson @{ tag :: String, value :: Json } json of
    Right { tag, value } | tag == reflectSymbol (Proxy @k) -> inj (Proxy @k) <$> decodeJson @v value
    _ -> expandCons @k <$> decodeJson_Variant @r0 @r @rl json

expandCons :: forall @k v r r'. Cons k v r r' => Variant r -> Variant r'
